...
A Risk (`risks.md`) already carries the narrative — what is genuinely new or uncertain, and why. Evidence does not restate that narrative: it is the compact, current pending/passed/failed status of the specific commit and run(s) that settle it, cross-referenced by Risk ID when one exists. An open Risk naming the pending surface is strong evidence for the dependency-safety check (`next-work-selection` skill) but not the sole gating mechanism — the check also weighs a plausible failure's actual effect on the next Feature's own implementation, and does not treat shared codebase or subsystem proximity alone as disqualifying.
