...
The following remain deliberately open, not settled by this document: the persistence implementation; cloud/sync architecture; provider mappings behind Allegro/Virtuoso/Maestro; frontend design-system, component, state-management, and rendering choices; and the great majority of the concrete protocol/schema messages a real product needs (EP-005's `project.proto` resolves only a first, minimal one-track/one-clip slice of this, not the general case; the new Desktop-facing schema ADR-010 introduces is equally minimal). ADR-010 resolves only how Desktop reaches Python core mechanically (the transport) — not which concrete UI-facing commands exist beyond a proof-of-mechanism message, nor any frontend design-system, component, state-management, or rendering choice, all of which stay open here.
